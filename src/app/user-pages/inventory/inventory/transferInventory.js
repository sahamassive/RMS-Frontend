...
function TransferInventory() {
  ...
  const changeQuantity = (event, id) => {
    ...
    const foundItem = inventoryQueue.find(
      (item) => item[0].ingredient_id == id
    );
    if (foundItem) {
      updatedList = inventory.filter((item) => {
        if (id == item.ingredient_id) {
          setVal(foundItem[0].askQuantity);
          let p =
            parseFloat(item.current_quantity) +
            parseFloat(foundItem[0].askQuantity);

          item.current_quantity = p - parseFloat(event);
        }
  ...
  const AddToQueue = (data) => {
    ...
    if (askQuantity == null) {
      ...
      const foundItem = inventoryQueue.find(
        (item) => item[0].ingredient_id == checkid
      );
      if (foundItem) {
        setPrev(0);

        return (foundItem[0].askQuantity = askQuantity);
      } else {
        singleQueue.push({
