...
function Waste() {
    ...
    const modalOpen = (employee_id) => {
        axios.get(`${baseUrl}/api/details-wastes/${employee_id}`).then((response) => {
                                ...
                                <table id="branchs" className="table table-striped table-style">
                                    ...
                                    <tbody>
                                        {allData.map((data, index) => (
                                            <tr>
                                                <td>{index + 1}</td>
                                                <td><img src={`${baseUrl}/foods/small/${data.image}`} width="50rem" alt={data.iamge}></img></td>
                                                <td>{data.name}</td>
                                                <td>{data.reason}</td>
                                                <td>
                                                    <button
            ...
            >
                <div className='emp-modal'>
                    {detailsData ?
                        <div>
                            <div className='btn-section'>
                                <img className='img-resize' width="100rem" src={`${baseUrl}/employee/medium/${detailsData[0].image}`} alt={detailsData[0].iamge}></img>
                                <div className='emp-style'>
                                    <h3>{detailsData[0].first_name} {detailsData[0].last_name}</h3>
                                    ...
                                    <table id="branchs" className="table table-striped table-hover">
                                        ...
                                        <tbody>
                                            {detailsData.map((data, index) =>
                                                <tr>
                                                    <td>{index + 1}</td>
                                                    <td><img src={`${baseUrl}/foods/small/${data.food_image}`} width="50rem" alt={data.food_iamge}></img></td>
                                                    <td>{data.name}</td>
                                                    <td>
